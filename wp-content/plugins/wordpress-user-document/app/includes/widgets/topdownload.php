...
/**
 * Class WUD_Topdownload_Widget
 */
class WUD_Topdownload_Widget extends WP_Widget {
	/**
	 * WUD_Topdownload_Widget constructor.
	 */
	public function __construct() {
		$widget_ops = array(
			'description'                 => esc_html__( 'Display Top Download Documents', 'wud' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'wud_topdownload_documents_widget', esc_html__( 'WUD - Top Download Documents', 'wud' ), $widget_ops );
	}
	...
	public function form( $instance ) {
		...
		$instance = wp_parse_args( $instance,
			array(
				'title'       => esc_attr__( 'Top Download Documents', 'wud' ),
				'category_id' => '',
				'limit'       => 5,
			)
		);
		...
		?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html__( 'Title', 'wud' ); ?> : </label>
	...
	public function widget( $args, $instance ) {
		...
		$docs       = $docs_model->get_top_download_documents( $category_id, $limit );
		...
		?>
		<?php
		include wud_get_template( 'widgets/topdownload', false, false );
		?>
		<?php

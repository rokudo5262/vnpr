...
class WPSTHEMES extends WPSHAPERE{
            ...
            {
              add_submenu_page( 'wpshapere-options', __('Admin Theme Presets', 'wps'), __('Admin Theme Presets', 'wps'), 'manage_options', 'wps_themes', array($this,'wps_theme_page') );
            }
    ...
    <div class="wrap wps-wrap">
            <h1><?php echo __('Import Admin Theme', 'wps'); ?></h1>
            <?php parent::wps_help_link(); ?>
            <p><?php echo __( 'Note: Importing a theme will replace your existing custom set colors.', 'wps') ?></p>
    <?php
    if(isset($_GET['status']) && $_GET['status'] == 'updated')
            echo '<div id="message" class="updated below-h2"><p>' . __( 'Theme Imported!', 'wps' ) . '</p></div>';
    ?>
        <form class="wps_set_admin_theme" name="wps_set_theme" method="post" action="">
          <div class="wps-admintheme-presets-wrap">
            <?php
                ...
                <input type="submit" class="button button-primary button-hero" name="submit" value="<?php _e('Import Theme', 'wps'); ?>" />

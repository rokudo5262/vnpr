...
class BP2WP_Full_Sync_Loader {
	...
	public function __construct() {
		...
		// Return if BuddyPress is not Active (Enhance this with an alert message)
	...
	public function admin_init() {
		// Add required hooks
		add_action( 'user_register', array( &$this, 'bp2wp_user_register' ), 10, 1 );
		add_action( 'personal_options_update', array( &$this, 'bp2wp_profile_update') );
		add_action( 'edit_user_profile_update', array( &$this, 'bp2wp_profile_update') );
	...
	/**
	 * Intercept the user_register action, storing the new user id WordPress field and syncing user the first time.
	 *
	...
	public function bp2wp_user_register( $user_id ) {
		...
		foreach ( $_POST as $key => $value ) {
			...
			// Correct the url key. Since that the input field form name is different from the database field name (url vs user_url).
			if ( $key == 'url' )
				$key = 'user_url';

			// Check for a xprofile field
			$parent_profile_id = intval( self::bp2wp_get_parent_fields( $key, 'id' ) );
			
			// If there is a corresponding xprofile field
			if ( $parent_profile_id && $parent_profile_id > 0 ) {
				xprofile_set_field_data( $parent_profile_id, $user_id, $value );
			}
		}
	}

	/**
	 * Intercept the WordPress Backend profile update.
	 *
	 * @since BP2WP Full Sync 0.3.6
	 */	
	public function bp2wp_profile_update( $user_id ) {
		...
		if ( current_user_can( 'edit_user', $user_id ) ) {
			// Check for a xprofile field
			$parent_profile_id = intval( self::bp2wp_get_parent_fields( 'user_url', 'id' ) );

			// If there is a corresponding xprofile field
			if ( $parent_profile_id && $parent_profile_id > 0 )
				xprofile_set_field_data( $parent_profile_id, $user_id, $_POST['url'] );
		}
	}
	...
	public function bp2wp_update_sync_field( $value, $field, $field_type_obj ) {
		if ( $field ) {
			...
			if (isset($childs) && $childs && count($childs) > 0 
				...
				// If the field is url update the users table instead the usermeta one
				if ( $childs[0]->name == 'user_url' ) {
					wp_update_user( array( 'ID' => $this->bp_user_id, $childs[0]->name => $value ) );
				}
				// Else update the usermeta table
				else {
		...
		foreach ( $posted_field_ids as $field_id ) {
			if ( $new_values[ $field_id ]['value'] !=  $old_values[ $field_id ]['value'] ) {
				...
				if (isset($childs) && $childs && count($childs) > 0 
					...
					// If the field is user_url update the users table instead the usermeta one
					if ( $childs[0]->name == 'user_url' ) {
						wp_update_user( array( 'ID' => $user_id, $childs[0]->name => strip_tags( $new_values[ $field_id ]['value'] ) ) );
					}
					// Else update the usermeta table
					else {

...
class NotificationX_Installer {
    ...
    public function migrator() {
		...
		if( ! isset( $plugins['notificationx/notificationx.php'] )) {
            if ( $this->make_notificationx_ready() ) {
                // redirect to plugin dashboard
                wp_safe_redirect( "admin.php?page=nx-admin" );
            }
        }
    }

    protected function make_notificationx_ready(){
        $basename = 'notificationx/notificationx.php';
        $is_plugin_installed = $this->get_installed_plugin_data( $basename );
        $plugin_data = $this->get_plugin_data( 'notificationx', $basename );
        ...
        if( $is_plugin_installed ) {
            ...
            if ( is_plugin_active( $basename )) {
                return delete_transient( 'nx_free_version' );
            } else {
            ...
            $download_link = isset( $plugin_data->download_link ) ? $plugin_data->download_link : NOTIFICATIONX_FREE_PLUGIN;

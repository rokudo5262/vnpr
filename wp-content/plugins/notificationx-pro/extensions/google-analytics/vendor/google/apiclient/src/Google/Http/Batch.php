 ...
 *
 * Note that calls to `NxPro_Google_Http_Batch::execute()` do not clear the queued
 * requests. To start a new batch, be sure to create a new instance of this
 * class.
 */
class NxPro_Google_Http_Batch
{
...
EOF;
    ...
    /** @var NxPro_Google_Http_Request $req */
    ...
    if (!empty($body)) {
      ...
      foreach ($parts as $i => $part) {
        ...
        if (!empty($part)) {
          ...
          try {
            $response = NxPro_Google_Http_REST::decodeHttpResponse($response, $requests[$i-1]);
          } catch (NxPro_Google_Service_Exception $e) {
